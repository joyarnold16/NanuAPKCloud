...
for name in "${required_env[@]}"; do
  if [[ -z "${!name:-}" ]]; then
    echo "Missing required Play release secret: $name" >&2
    exit 2
  fi
done
...
# signing material is written to the repository.
python3 <<'PY'
...
# Build and validate the complete RC8/1.0 codebase first. This leaves a fully
# generated Android project under llama-upstream/examples/llama.android.
...
# Verify that the AAB is JAR-signed with the upload key and that its package
# resources include the production reporting configuration.
