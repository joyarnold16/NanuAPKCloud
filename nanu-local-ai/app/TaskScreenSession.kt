    ...
    fun submit(prompt: String, system: String, request: JSONObject = JSONObject(), attachment: NanuAttachment? = null) {
        ...
        screen.lifecycleScope.launch {
            try {
                ...
                val agent = !image && !request.optBoolean("groundedOnly")
                request.put("prompt", body)
                    .put("userPrompt", prompt)
                    .put("system", system + if(agent) ProStore.assistantInstructions(screen) else "")
                    .put("model", prefs.getString("last_model", ""))
                if(agent) {
