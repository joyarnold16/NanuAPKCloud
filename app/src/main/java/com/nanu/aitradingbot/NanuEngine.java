...
public class NanuEngine {
    ...
    public NanuEngine(AppStore s) {
        store = s;
        seed();
        addJournal("Nanu v5.6.1 Controlled Live Dry-Run + Order Safety Engine ready in Paper Mode.");
    }
    ...
    public void start() {
        ...
        if ("live".equals(store.mode) && store.liveUnlocked && store.liveDryRunEnabled) {
            addJournal("LIVE DRY-RUN active: Nanu will preview and validate orders only; no real orders sent in v5.6.1.");
        }
        tick(true);
    }
    ...
    private void buildBrain() {
        ...
        brain.add("Order Safety: " + (store.liveDryRunEnabled ? "Controlled dry-run ON; real live orders blocked in v5.6.1." : "Dry-run OFF; do not use live until reviewed."));
