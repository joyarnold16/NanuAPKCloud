...
public class MainActivity extends Activity {
    ...
    void buildSecurity() {
        ...
        addGap(box, 8);
        box.addView(actionButton("Unlock LIVE Dry-Run Gate", RED, v -> unlockLive()), new LinearLayout.LayoutParams(-1, dp(54)));
        box.addView(tv("LIVE can be selected for API Doctor. In v5.6.1, unlock allows controlled dry-run only; real orders are still blocked.", 12, AMBER, false));
        addGap(box, 18);
        ...
        box.addView(actionButton("Test Phone Alert / Long Sound", AMBER, v -> { store.triggerAlert("Nanu Phone Alert Test", "Phone notification, sound and vibration test from Nanu.", true); toast("Phone alert test sent"); }), new LinearLayout.LayoutParams(-1, dp(52)));
        root.addView(box);
    }
    ...
    void buildApiDoctorStatus(LinearLayout box) {
        ...
        status.addView(tv("Live auto trading stays blocked until API Doctor + Telegram Doctor + Profit Guard + Panic test + withdrawal OFF confirmation pass.", 12, AMBER, false));
        box.addView(status);
    }
    ...
    void buildLiveChecklist(LinearLayout box) {
        ...
        list.addView(tv("v5.6.1 only allows controlled live dry-run and order preview. Real order placement remains blocked until a later audited build.", 11, AMBER, false));
        box.addView(list);
    }
    ...
    void buildOrderSafetyCard() {
        ...
        box.addView(tv("v5.6.1 previews live orders, checks safety rules, rounds quantity and logs the result. It does not send real live orders.", 12, MUTED, false));
        ...
        if (!store.lastOrderSymbol.isEmpty()) box.addView(tv("Last preview: " + store.lastOrderSymbol + "  •  " + (store.lastOrderSafetyPass ? "PASS" : "BLOCKED"), 12, store.lastOrderSafetyPass ? GREEN : AMBER, true));
        addGap(box, 10);
        LinearLayout buttons = row();
    ...
    void buildOrderSafetySettings(LinearLayout box) {
        ...
        box.addView(tv("Controlled live dry-run, order preview, minimum order checks, cooldown and max trades/day guard. Real order placement is still blocked in v5.6.1.", 12, MUTED, false)); addGap(box, 8);
        LinearLayout r1 = row();
        ...
        box.addView(tv("First real order confirmation remains required. In v5.6.1, even after LIVE unlock, Nanu only performs dry-run order validation and will not submit real orders. Dry-run previews do not create real open trades.", 11, AMBER, false)); addGap(box, 18);
    }

    void runDryRunPreview() {
        OrderSafetyEngine.Preview p = OrderSafetyEngine.buildDryRunPreview(store);
        if (p.pass) store.triggerAlert("Nanu Dry-Run PASS", "Order preview passed for " + p.symbol + ". No real order was sent.", false);
        else store.triggerAlert("Nanu Dry-Run Blocked", "Order preview was blocked by safety checks. No real order was sent.", true);
        alert("Live Dry-Run Order Preview", p.report);
        render(false);
    }

    String orderSafetyGuide() {
        return "Controlled Live Dry-Run:\nNanu uses real mode/risk settings to preview an order, but v5.6.1 does not send real Binance orders.\n\n" +
    ...
    void startBot() {
        ...
        store.engine.start();
        store.triggerAlert("Nanu Started", "Bot started in " + store.mode.toUpperCase(Locale.US) + ("live".equals(store.mode) ? " controlled dry-run mode. No real orders sent in v5.6.1." : " mode."), false);
    ...
    void stopBot() { store.engine.stop(); stopService(new Intent(this, NanuBotService.class)); store.triggerAlert("Nanu Stopped", "Bot stopped safely.", false); toast("Nanu stopped"); render(false); }
    void panic() { store.engine.panicClose(); store.panicButtonTested = true; store.save(); stopService(new Intent(this, NanuBotService.class)); store.triggerAlert("Nanu PANIC", "Panic close activated. Check Binance manually before restarting.", true); toast("Panic close activated"); render(false); }
    ...
    void unlockLive() {
        ...
        if (!missing.isEmpty()) {
            alert("Live Dry-Run Unlock Blocked", "Nanu will not unlock the live dry-run gate yet. Fix these items first:\n\n" + missing + "\nThis is intentional safety protection for your Binance account.");
            return;
        }
        input("Unlock LIVE Dry-Run Gate", "Type UNLOCK LIVE", "", false, s -> { if ("UNLOCK LIVE".equals(s.trim())) { store.liveUnlocked=true; store.mode="live"; store.save(); store.triggerAlert("Nanu Live Dry-Run Unlocked", "LIVE dry-run gate unlocked. v5.6.1 still sends no real orders.", true); toast("Live dry-run gate unlocked"); } else toast("Live not unlocked"); render(true); });
    }
    ...
    String liveMissingItems() {
        ...
        if (!store.liveDryRunEnabled) m.append("• Keep Controlled Live Dry-Run ON for v5.6.1.\n");
    ...
    void testApi() {
        ...
        BinanceClient.testApi(store, result -> runOnUiThread(() -> {
            ...
            if (!store.apiDoctorOkForCurrentMode()) store.triggerAlert("Nanu API Doctor Failed", "API Doctor did not pass for " + store.mode.toUpperCase(Locale.US) + ". Check app diagnosis.", true);
    ...
    void footer() { addGap(14); TextView f = tv("Nanu AI Trading Bot v5.6.1 • Final Safety Polish • One Dialog • Live Checklist", 11, MUTED, false); f.setGravity(Gravity.CENTER); root.addView(f); }
