...
public class MainActivity extends Activity {
    ...
    void buildApiDoctorStatus(LinearLayout box) {
        ...
        status.addView(tv("Private API: " + (store.lastApiPrivateOk ? "OK" : "Not OK / Not tested"), 12, store.lastApiPrivateOk ? GREEN : AMBER, true));
        status.addView(tv("Spot trading permission: " + (store.lastApiCanTrade ? "OK" : "Blocked / Read-only"), 12, store.lastApiCanTrade ? GREEN : RED, true));
    ...
    void startBot() {
        ...
        if ("live".equals(store.mode) && !store.lastApiCanTrade) { toast("Live auto trading blocked: API Doctor has not confirmed spot trading permission."); activeTab=4; render(true); return; }
        store.engine.start();
    ...
    void unlockLive() {
        ...
        if (!store.lastApiPrivateOk) { toast("Run API Doctor first. Private API must be OK."); return; }
        if (!store.lastApiCanTrade) { toast("Live blocked: key is read-only or spot trading permission is OFF."); alert("Live Blocked", "API Doctor has not confirmed spot trading permission. Add trusted IP in Binance, enable Spot & Margin & Stock Trading, keep Withdrawals OFF, then run API Doctor again."); return; }
    ...
    void testApi() { alert("API Doctor", "Testing Binance connection, public IP, signature and private account permission. Please wait..."); BinanceClient.testApi(store, result -> runOnUiThread(() -> { alert("Nanu API Doctor", result); render(false); })); }
    ...
    void footer() { addGap(14); TextView f = tv("Nanu AI Trading Bot v5.3 • API Doctor • Trusted IP Helper • Live locked", 11, MUTED, false); f.setGravity(Gravity.CENTER); root.addView(f); }
