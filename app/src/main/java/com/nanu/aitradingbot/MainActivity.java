...
public class MainActivity extends Activity {
    ...
    void buildSecurity() {
        ...
        box.addView(tv("LIVE can be selected for API Doctor. In v5.6, unlock allows controlled dry-run only; real orders are still blocked.", 12, AMBER, false));
        addGap(box, 18);
    ...
    void buildLiveChecklist(LinearLayout box) {
        ...
        list.addView(tv("v5.6 only allows controlled live dry-run and order preview. Real order placement remains blocked until a later audited build.", 11, AMBER, false));
        box.addView(list);
    }
    ...
    void buildOrderSafetyCard() {
        ...
        box.addView(tv("v5.6 previews live orders, checks safety rules, rounds quantity and logs the result. It does not send real live orders.", 12, MUTED, false));
    ...
    void buildOrderSafetySettings(LinearLayout box) {
        ...
        box.addView(tv("Controlled live dry-run, order preview, minimum order checks, cooldown and max trades/day guard. Real order placement is still blocked in v5.6.", 12, MUTED, false)); addGap(box, 8);
        LinearLayout r1 = row();
        ...
        box.addView(tv("First real order confirmation remains required. In v5.6, even after LIVE unlock, Nanu only performs dry-run order validation and will not submit real orders.", 11, AMBER, false)); addGap(box, 18);
    }
    ...
    String orderSafetyGuide() {
        return "Controlled Live Dry-Run:\nNanu uses real mode/risk settings to preview an order, but v5.6 does not send real Binance orders.\n\n" +
    ...
    void startBot() {
        ...
        store.engine.start();
        store.triggerAlert("Nanu Started", "Bot started in " + store.mode.toUpperCase(Locale.US) + ("live".equals(store.mode) ? " controlled dry-run mode. No real orders sent in v5.6." : " mode."), false);
    ...
    void unlockLive() {
        ...
        input("Unlock LIVE Dry-Run Gate", "Type UNLOCK LIVE", "", false, s -> { if ("UNLOCK LIVE".equals(s.trim())) { store.liveUnlocked=true; store.mode="live"; store.save(); store.triggerAlert("Nanu Live Dry-Run Unlocked", "LIVE dry-run gate unlocked. v5.6 still sends no real orders.", true); toast("Live dry-run gate unlocked"); } else toast("Live not unlocked"); render(true); });
    }
    ...
    String liveMissingItems() {
        ...
        if (!store.liveDryRunEnabled) m.append("• Keep Controlled Live Dry-Run ON for v5.6.\n");
    ...
    void footer() { addGap(14); TextView f = tv("Nanu AI Trading Bot v5.6 • Final Safety Polish • One Dialog • Live Checklist", 11, MUTED, false); f.setGravity(Gravity.CENTER); root.addView(f); }
