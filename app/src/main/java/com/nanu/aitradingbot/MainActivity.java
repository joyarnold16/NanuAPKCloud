...
public class MainActivity extends Activity {
    ...
    void buildSecurity() {
        ...
        box.addView(tv("Trading mode, API Doctor, trusted IP helper, key permissions, risk shield, coin selection and live unlock.", 13, MUTED, false)); addGap(box, 14);
        ...
        box.addView(actionButton("Run API Doctor", GREEN, v -> testApi()), new LinearLayout.LayoutParams(-1, dp(56))); addGap(box, 8);
        ...
        addGap(box, 8);
        box.addView(actionButton("Unlock LIVE Auto Trading", RED, v -> unlockLive()), new LinearLayout.LayoutParams(-1, dp(54)));
        box.addView(tv("LIVE can be selected for API Doctor. Auto trading remains locked until this unlock button passes all checks.", 12, AMBER, false));
        addGap(box, 18);
        ...
        tele.addView(actionButton(store.telegramToken.isEmpty() ? "Bot Token" : "Bot Token Saved", CYAN, v -> input("Telegram Bot Token", "Paste token from BotFather", store.telegramToken, false, t -> { store.telegramToken=t.trim(); store.save(); render(true); })), new LinearLayout.LayoutParams(0, dp(52), 1));
        LinearLayout.LayoutParams chat = new LinearLayout.LayoutParams(0, dp(52), 1); chat.leftMargin=dp(10);
        tele.addView(actionButton(store.telegramChatId.isEmpty() ? "Chat ID" : "Chat ID Saved", CYAN, v -> input("Telegram Chat ID", "Paste your chat id", store.telegramChatId, false, cid -> { store.telegramChatId=cid.trim(); store.save(); render(true); })), chat);
        box.addView(tele); addGap(box, 8);
        box.addView(actionButton("Run Telegram Doctor / Test Message", GREEN, v -> testTelegram()), new LinearLayout.LayoutParams(-1, dp(54)));
        root.addView(box);
    }
    ...
    void buildApiDoctorStatus(LinearLayout box) {
        ...
        status.addView(tv("Live auto trading stays blocked until API Doctor confirms private access + spot trading permission.", 12, AMBER, false));
        box.addView(status);
    }
    ...
    void startBot() {
        if ("live".equals(store.mode) && !store.liveUnlocked) { toast("Live auto trading locked. Use Unlock LIVE Auto Trading after API Doctor passes."); activeTab=4; render(true); return; }
        ...
        store.engine.start();
        store.triggerAlert("Nanu Started", "Bot started in " + store.mode.toUpperCase(Locale.US) + " mode.", false);
    ...
    void panic() { store.engine.panicClose(); stopService(new Intent(this, NanuBotService.class)); store.triggerAlert("Nanu PANIC", "Panic close activated. Check Binance manually before restarting.", true); toast("Panic close activated"); render(false); }
    ...
    void unlockLive() {
        ...
        if (!store.apiDoctorOkForCurrentMode()) { toast("Run API Doctor in LIVE mode first. Private API must be OK."); return; }
        if (!store.apiTradingOkForCurrentMode()) { toast("Live blocked: key is read-only or spot trading permission is OFF."); alert("Live Blocked", "API Doctor has not confirmed spot trading permission in LIVE mode. Add trusted IP in Binance, enable Spot & Margin & Stock Trading only, keep Withdrawals OFF, then run API Doctor again."); return; }
        input("Unlock LIVE Auto Trading", "Type UNLOCK LIVE", "", false, s -> { if ("UNLOCK LIVE".equals(s.trim())) { store.liveUnlocked=true; store.mode="live"; store.save(); store.triggerAlert("Nanu Live Unlocked", "LIVE auto trading was unlocked. Start small and monitor closely.", true); toast("Live auto trading unlocked"); } else toast("Live not unlocked"); render(true); });
    }
    void testApi() { alert("API Doctor", "Testing Binance connection, public IP, signature and private account permission for " + store.mode.toUpperCase(Locale.US) + ". Please wait..."); BinanceClient.testApi(store, result -> runOnUiThread(() -> { if (!store.apiDoctorOkForCurrentMode()) store.triggerAlert("Nanu API Doctor Failed", "API Doctor did not pass for " + store.mode.toUpperCase(Locale.US) + ". Check app diagnosis.", true); alert("Nanu API Doctor", result); render(false); })); }
    void showPublicIp() { alert("Public IP", "Checking your public IP. Please wait..."); BinanceClient.getPublicIp(store, result -> runOnUiThread(() -> { alert("Trusted IP Helper", result); render(false); })); }
    void testTelegram() { alert("Telegram Doctor", "Sending Telegram test message. Please wait..."); TelegramClient.test(store, result -> runOnUiThread(() -> alert("Telegram Doctor", result))); }

    String developerReport() { return "Nanu AI Trading Bot v5.4\nMode: " + store.mode + "\nRunning: " + store.engine.running + "\nPanic: " + store.engine.panic + "\nCoin mode: " + (store.autoCoinMode?"Auto":"Manual") + "\nWatchlist: " + store.watchlist + "\nOpen trades: " + store.engine.trades.size() + "\nLive unlocked: " + store.liveUnlocked + "\nProfit Guard: " + store.profitGuardEnabled + " / target " + store.profitTargetUsdt + " USDT" + "\nDuplicate Guard: " + store.duplicateProfitGuardEnabled + " / repeats " + store.sameProfitRepeats + "/" + store.duplicateProfitRepeatCount + "\nAPI last mode: " + store.lastApiMode + "\nAPI HTTP: " + store.lastApiHttpCode + "\nAPI private OK: " + store.lastApiPrivateOk + "\nAPI can trade: " + store.lastApiCanTrade + "\nPublic IP: " + store.lastPublicIp + "\nAPI diagnosis: " + store.lastApiDiagnosis; }
    ...
    void footer() { addGap(14); TextView f = tv("Nanu AI Trading Bot v5.4 • Live Gate Fix • Telegram Doctor • Safety Lock", 11, MUTED, false); f.setGravity(Gravity.CENTER); root.addView(f); }
