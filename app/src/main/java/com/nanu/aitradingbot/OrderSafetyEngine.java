...
public class OrderSafetyEngine {
    ...
    public static Preview buildDryRunPreview(AppStore store) {
        ...
        out.append("Nanu Order Safety Engine v5.6\n\n");
        ...
        ok &= check(out, "Max open trades not exceeded", store.engine.trades.size() <= Math.max(1, store.maxOpenTrades));
        ...
        if (ok) {
            out.append("Nanu is allowed to simulate the order path only. Real order placement remains blocked in v5.6.\n");
        ...
        out.append("\nSafety note: v5.6 does not place real live orders. It previews, validates and logs the order plan.");
