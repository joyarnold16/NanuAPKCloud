    ...
    public static void testApi(AppStore store, Callback cb) {
        exec.execute(() -> {
            ...
            try {
                ...
                out.append("Nanu API Doctor v5.3\n\n");
                ...
                out.append("Key rule: ").append(keyRule(store.mode)).append("\n\n");
                ...
                if (code == 200) {
                    ...
                    out.append("Deposit flag: ").append(canDeposit ? "ON" : "OFF").append('\n');
                    out.append("Withdraw flag: ").append(canWithdraw ? "ON ⚠️" : "OFF ✅").append("\n\n");
                    if (!canTrade) {
                        ...
                        if (canWithdraw) out.append("Warning: Withdrawal permission should be OFF for Nanu. Disable it before live use.\n");
                    }
                } else {
                    store.lastApiPrivateOk = false;
                    store.lastApiCanTrade = false;
                    store.save();
                    String diagnosis = diagnosisFor(code, body, store.mode);
